...
enum Mode {
    ...
    static func switchTo(_ mode: Mode) {
        ...
        if let currentMode = d.string(forKey: USERDEFAULTS_SHADOWSOCKS_RUNNING_MODE), let acl = d.string(forKey: USERDEFAULTS_ACL_FILE_NAME) {
            switch mode {
                case .PACNETEASE:
                    if currentMode == "autoWithNetease" { return }
                case .PAC:
        ...
        switch mode {
            case .PACNETEASE:
                d.setValue("autoWithNetease", forKey: USERDEFAULTS_SHADOWSOCKS_RUNNING_MODE)
        ...
        d.synchronize()
        SyncSSLocal (finish: { (suc) in
            MainMenuManager.applyConfig { (suc) in
                NotificationCenter.default.post(name: NOTIFY_UPDATE_RUNNING_MODE_MENU, object: d.string(forKey: USERDEFAULTS_SHADOWSOCKS_RUNNING_MODE)!)
            }
        }, runningModel: d.string(forKey: USERDEFAULTS_SHADOWSOCKS_RUNNING_MODE)!)
    }
}
